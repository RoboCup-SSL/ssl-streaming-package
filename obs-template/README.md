...
|---|---|
| `command_color` | tinted to the team of a team-specific stoppage (ball placement, timeout, kickoff/penalty prep, goal) — blue `#779fff` / yellow `#fff145`; **transparent** during running play (incl. a taken free kick) and team-less commands, so a backing bar hides itself. Put it behind `command`/`game_state` for a team-coloured highlight. |
