...
// The pattern's flat front is at the top, so it faces 'up' un-rotated; rotate(90)
// turns it to face 'right' (the direction of travel used in the chase stingers).
export const RobotTop: React.FC<{
  ...
  bodyColor?: string;
  orientation?: 'up' | 'right'; // which way the flat front points (default 'right' = travelling)
}> = ({size, teamColor, idDots, bodyColor = '#1a1a1e', orientation = 'right'}) => {
  const [fl, fr, rl, rr] = idDots;
  return (
    <svg width={size} height={size} viewBox="-85 -85 170 170" style={{display: 'block'}}>
      <g transform={`rotate(${orientation === 'right' ? 90 : 0})`}>
